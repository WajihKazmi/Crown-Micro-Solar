...
class DeviceEnergyFlowModel {
  ...
  // Attempt to derive PV voltage - returns cumulative voltage (PV1 + PV2) for dual PV devices
  // This matches old app behavior (deviceinformation.dart line 3009: _Solar_pv1! + _Solar_pv2!)
  double? get pvVoltage {
    ...
    for (final p in pvStatus) {
      ...
      if ((unit == 'v' || par.contains('volt')) && p.value != null) {
        sumV += p.value!;
        count++;
      }
    }
    return count > 0 ? sumV : null;
  }

  // Attempt to derive PV current - returns cumulative current (PV1 + PV2) for dual PV devices
  double? get pvCurrent {
    double sumI = 0;
    int count = 0;
    for (final p in pvStatus) {
      final unit = (p.unit ?? '').toLowerCase();
      final par = (p.par).toLowerCase();
      if ((unit == 'a' || par.contains('current')) && p.value != null) {
        sumI += p.value!;
        count++;
      }
    }
    return count > 0 ? sumI : null;
  }
...
// Lightweight in-process cache with TTL for hot device endpoints
class _TimedCache<V> {
  final Duration ttl;
...
class DeviceRepository {
  ...
  // Logical metrics: PV_OUTPUT_POWER, LOAD_POWER, GRID_POWER, BATTERY_SOC, AC2_OUTPUT_VOLTAGE, AC2_OUTPUT_CURRENT, PV_INPUT_VOLTAGE, PV_INPUT_CURRENT, GRID_FREQUENCY, BATTERY_CAPACITY, BATTERY_VOLTAGE, BATTERY_CHARGING_CURRENT, BATTERY_DISCHARGE_CURRENT, GENERATOR_AC_VOLTAGE, UTILITY_AC_VOLTAGE, PV1_CHARGING_POWER, PV2_CHARGING_POWER, AC_OUTPUT_ACTIVE_POWER, PV1_INPUT_VOLTAGE, PV2_INPUT_VOLTAGE, PV1_INPUT_CURRENT, PV2_INPUT_CURRENT, TODAY_GENERATION, TOTAL_GENERATION, INPUT_POWER
  ...
  // Short-lived cache for energy flow responses per PN to avoid duplicate hits
  _TimedCache<DeviceEnergyFlowModel>? _flowCache;
  ...
  // Resolve a metric aggregated per-year totals using SP endpoint.
  Future<MetricResolutionResult> resolveMetricTotalPerYear({
    required String logicalMetric,
    required String sn,
    required String pn,
    required int devcode,
  ...
  Future<Map<String, dynamic>> getDevicesAndCollectors(String plantId) async {
    ...
    // Build device and collector requests in parallel to reduce latency
    final deviceAction =
    ...
    final responses = await Future.wait([
      _apiClient.signedPost(deviceUrl),
      _apiClient.signedPost(collectorUrl),
    ]);
    final deviceJson = json.decode(responses[0].body);
    final collectorJson = json.decode(responses[1].body);
    ...
    for (var i = 0; i < collectors.length; i += maxConcurrent) {
      ...
      final batchTasks = batch.map((collector) async {
        ...
        try {
          print('DeviceRepository: Fetching devices for collector PN=$pn');
          final subDevices = await getDevicesForCollector(pn).timeout(timeout);
  ...
  Future<Map<String, dynamic>> getDevicesAndCollectorsQuick(
    ...
    // Fetch subordinate device info with quick timeout (max 3 seconds total)
    final maxConcurrent = 5; // Faster parallel fetching
    ...
    for (var i = 0; i < collectors.length; i += maxConcurrent) {
      ...
      final batchTasks = batch.map((collector) async {
        ...
        try {
          ...
          // On timeout/error, continue with empty list
          collectorDevices[pn] = [];
        }
      }).toList();
    ...
    // Filter standalone: exclude subordinate devices AND collectors themselves
    final standaloneDevices = devices
        .where((d) => !subordinateSNs.contains(d.sn) && !d.isCollector)
        .toList();
